 ...
 * Если ввели неправильный тип питомца, то программа выводит: «Incorrect input. Unsupported pet type»
 * * и переходит к следующему вводу.
 * Если ввели отрицательный или нулевой возраст, то программа выводит: «Incorrect input. Age <= 0»
 * * и переходит к следующему вводу.
 * Если ввели отрицательную или нулевую массу, то программа выводит: «Incorrect input. Mass <= 0»
 * * и переходит к следующему вводу.
 * Программа не завершается с ошибкой при некорректных входных данных.
 * * Она выводит: «Could not parse a number. Please, try again» и повторяет попытку ввода.
...
public class Main {
    ...
    public static Animal addPet(PetCharact pet) {
        ...
        return switch (pet.type) {
            case "dog" -> new Dog(pet.name, pet.age, pet.mass);
            case "cat" -> new Cat(pet.name, pet.age, pet.mass);
            default -> new Cat("-999", -999, -999.999);
        };
    }
}